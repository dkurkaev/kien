...
// slow wipe straight THROUGH the centre (world 0,0,0)
const A = await p.evaluate(()=>window.__project(-0.9,0.02,0));
const C = await p.evaluate(()=>window.__project(0.9,0.02,0));
await p.mouse.move(A[0],A[1]); await p.mouse.down();
for (let t=0;t<=1.01;t+=0.12){ await p.mouse.move(A[0]+(C[0]-A[0])*t, A[1]+(C[1]-A[1])*t); await sleep(90);} 
await p.mouse.up(); await sleep(150);
console.log('interact counts:', JSON.stringify(await p.evaluate(()=>window.__icounts())));
console.log('radii after through-centre wipe:', JSON.stringify(await p.evaluate(()=>window.__spillRadii())));
await b.close();
